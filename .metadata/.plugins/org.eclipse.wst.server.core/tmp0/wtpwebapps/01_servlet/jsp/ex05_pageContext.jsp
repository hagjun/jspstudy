...
<%--
	...
	JSP 페이지에서 1:1 로 연결된 객체 JSP 페이지당 하의 pageContext 객체가 생성
	같은 페이지에서 서로 값을 공유할 수 있으며 다른 기본내장객체를 구하거나 페이지의 흐름을 제어한다.
	1. 데이터 저장 : pageContext.setAttribute("이름", 객체);
	   ...
	   ** 속성을 이용해서 데이터를 저장 및 호출하는 내장 객체들
	     ...
	     - session     : 해당 브라우저가 종료되면 정보도 사라진다.
	                    (예외적으로 시간을 지정할 수 있다. => 팝업에서 일주일간 안보기 , 일정시간지나면 로그아웃)
	                     보통 로그인 처리할 때 사용
...
--%>
    ...
    %>
	<h2>결과 보기</h2>
	<h2>EL : pageContext, request, session, application 에 저장된 데이터 호출가능</h2>
	<h3>주소 : ${addr }</h3>    
	<h3>나이 : ${age }</h3>    
	<h3>성별 : ${gender }</h3>    
	<h3>str : ${str}</h3>  
