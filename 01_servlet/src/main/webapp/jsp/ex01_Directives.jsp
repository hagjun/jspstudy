...
<%--
	...
	<%@ page %>    : 해당 페이지를 톰캣이 처리하는데 필요한 정보를 기술하는 부분
    ...
    1. page 지시어 속성들 
     ...
     - buffer : 출력에 사용되는 임시저장 공간(기본: 8kb)
     - autoFlush : 출력버퍼가 다 차지 않아도 바로 출력 시켜줌(기본: true)
     - erroPage : 해당 페이지에서 오류가 발생하면 오류를 처리 할 때 페이지를 지정
     - isErrorPae : 해당 페이지가 오류를 처리하는 페이지이면 true를 지정한다.
     
     2. include <%@ include file="삽입할 파일위치와 이름" %>
     ** include 는 지시어를 사용하는 방법과 액션태그를 사용하는 방법 두가지가 있다.
     
     3. taglib : 해당 페이지에서 사용할 태그 라이브러리 지정
                 나중에 JSTL할 때 필요 (JSTL 있어야 반복문 처리할 수 있다.)
